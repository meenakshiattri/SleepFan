...
public class Fan1Activity extends Activity implements OnClickListener
{
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		MenuActivity.soundSelection=R.raw.air_conditioner_fan_with_loop;
		...
		fanBlade = new GameView(Fan1Activity.this);
		...
		startspin_btn = (Button) findViewById(R.id.startspin_btn);       //stop image
		stopspin_btn = (Button) findViewById(R.id.stopspin_btn);	     //start image  
		...
		if(sound!=null && MenuActivity.onTouchFan1)
		{
			sound.play();
		}
	}
		...
		case R.id.stopbtn:
			...
			if (onTouchStopBtn) 
			{
				...
				if(onTouchZeroSet)
				{
					...
					onTouchSetTime=false; 
					sound = null;
					sound = new SoundCrossFading(Fan1Activity.this);
					sound.load(R.raw.air_conditioner_fan_with_loop, true);
					sound.play();
			...
			timerkill.scheduleAtFixedRate(new TimerTask() 
			{
				...
				public void run() 
				{
					...
					if(!OnTimeChange)
					{
						...
						{
							System.out.println("value of time in else"+timeincrease);
							finish();
							if(timerkill!=null)
							{   
								timerkill.cancel();
